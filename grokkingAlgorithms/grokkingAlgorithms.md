...
* Пройдемся по списку в поисках исполнителя с самым большим количеством прослушиваний. Список еще не отсортирован, поэтому проверить необходимо каждый элемент списка, чтобы иметь возможность сравнивать прослушивания и найти самое большое число. После нахождения такового занесем этого исполнителя первым номером в новый список, который будет отражать результаты сортировки. В итоге, учитывая необходимость для обнаружения самого прослушиваемого исполнитьеля "просмотреть" каждый элемент списка n, получаем время выполнения как $O(n)$
* Теперь нам нужно найти следующего по популярности исполнителя - тот же алгоритм действий, то же время выполнения $O(n)$. 
* Начинает складываться понимание - чтобы произвести сортировку (по заданному параметру) списка элементов длинною n, нам необходимо пройтись по каждому элементу списка $O(n)$ такое число раз, которое точно равно количеству элементов в списке - n раз, а значит время выполнения $O(n)$. 
...
Так как у нас есть _неотсортированный_ подмассив из 2 элементов, мы просто рекурсивно вызовем для него нашу функцию сортировки. 
